...
class DvdEngine:
    ...
    def slerp(self, v0, v1, t):
        """Interpolación esférica para mezclar semillas sin perder energía de ruido."""
    ...
    def generar(self, prompt, neg_prompt="low quality, blurry, distorted, canvas", steps=15, width=1024, height=1024, cfg=7.5, seed=None, var_seed=None, var_strength=0.0):
        ...
        print(f"[*] Render: {width}x{height} | Seed: {seed} | Var: {var_strength}")
        ...
        metadata.add_text("Prompt", prompt); metadata.add_text("Seed", str(seed))
        metadata.add_text("CFG scale", str(cfg)); metadata.add_text("Steps", str(steps))
        if var_strength > 0:
